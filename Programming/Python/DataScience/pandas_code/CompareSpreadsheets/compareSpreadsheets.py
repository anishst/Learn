...
df1 = pd.read_excel('ss1.xlsx', 'Sheet1', na_values=['NA'])
df2 = pd.read_excel('ss2.xlsx', 'Sheet1', na_values=['NA'])

print(df1.head())
# Make sure we order by account number so the comparisons work
df1.sort_values(by=['account number'])
df1=df1.reindex()
df2.sort_values(by=['account number'])
df2=df2.reindex()
